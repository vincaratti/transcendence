...
- **Single-player mode** — play solo at your own pace
...
| **sbrugman** | Developer | Implemented user authentication flows, the initial frontend structure, the homepage/dashboard UI, and the friend-list chat panel (messaging in active development) |
...
| Avatar upload | sbrugman, arcornil | Upload PNG/JPEG/GIF/WebP (max 5 MB); stored on server; accessible at `/api/uploads/avatars/` |
...
| Single-player mode | arcornil | Play alone to learn the game; no opponent required |
...
| Chat panel | sbrugman | Friend list visible in pre-game panel; real-time messaging is in active development |
...
**User interaction (Major, 2pts):** A profile system displays user information and stats. A friends system allows adding users by username, accepting or declining requests, and viewing the full friend list with live presence indicators. A chat panel is implemented with the friend-list UI; real-time messaging is in active development.
...
**Standard user management (Major, 2pts):** Users register with email and password (bcrypt-hashed), log in to receive a JWT, update their profile information, upload a custom avatar, and see friends' online status in real time.
...
- Built the majority of the frontend: Dashboard, Codenames game view, Lobby, Multiplayer flow, Singleplayer mode, Stats, Leaderboard, Match History, Achievements display, StatsOverview
...
- Began work on the pre-game chat panel (`PreGameChat.vue`); the friend-list sidebar is functional; real-time messaging is in active development
